  ...
  useEffect(() => {
    ...
    const allLawyers = async () => {
      ...
      const filtered = data.filter((lawyer) => {
        if (!lawyer.feedback || lawyer.feedback.length === 0) {
          return false;
        }

        const total = lawyer.feedback.reduce((sum, fb) => sum + fb.rating, 0);
        const avgRating = total / lawyer.feedback.length;
      ...
      <div className="mt-52  w-[85%] mx-auto">
        ...
        <div className=" mt-5   grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {lawyers.slice(0, 4).map((lawyer) => (
            <div
              key={lawyer.id}
              className="relative w-fit mx-auto overflow-hidden rounded-xl shadow-lg"
            >
